...
typedef struct dns_table_entry
{
    ...
    uint32_t ipv4_be;        /* network byte order; 0 => BLOCK */
...
int dns_table_load_file(dns_table *t, const char *path)
{
    ...
    while (fgets(line, sizeof(line), fp) != NULL)
    {
        ...
        /* Strip leading/trailing whitespace (simple trim). */
        {
            char *p = line;
            while (*p == ' ' || *p == '\t' || *p == '\r' || *p == '\n')
            {
        ...
        /* Support inline comments: everything after '#' is ignored. */
        {
            char *hash = strchr(line, '#');
            if (hash != NULL)
            {
                /* Trim trailing whitespace before cutting. */
                while (hash > line &&
